...
class Xenics(Instrument, Camera):
    ...
    def load_dll(self):
        ...
        self.is_initialised.argtypes = (ctypes.c_int32,)

        self.start_capture = self.xenicsdll.XC_StartCapture
        self.start_capture.restype = ctypes.c_ulong  # ErrCode
        self.start_capture.argtypes = (ctypes.c_int32,)

        self.is_capturing = self.xenicsdll.XC_IsCapturing
        self.is_capturing.restype = ctypes.c_bool
        self.is_capturing.argtypes = (ctypes.c_int32,)

        self.get_frame_size_dll = self.xenicsdll.XC_GetFrameSize
        self.get_frame_size_dll.restype = ctypes.c_ulong
        self.get_frame_size_dll.argtypes = (ctypes.c_int32,)  # Handle

        self.get_frame_type_dll = self.xenicsdll.XC_GetFrameType
        self.get_frame_type_dll.restype = ctypes.c_ulong  # Returns enum
        self.get_frame_type_dll.argtypes = (ctypes.c_int32,)  # Handle

        self.get_frame_width = self.xenicsdll.XC_GetWidth
        self.get_frame_width.restype = ctypes.c_ulong
        self.get_frame_width.argtypes = (ctypes.c_int32,)  # Handle

        self.get_frame_height = self.xenicsdll.XC_GetHeight
        self.get_frame_height.restype = ctypes.c_ulong
        self.get_frame_height.argtypes = (ctypes.c_int32,)  # Handle

        self.get_max_val = self.xenicsdll.XC_GetMaxValue
        self.get_max_val.restype = ctypes.c_ulong
        self.get_max_val.argtypes = (ctypes.c_int32,)  # Handle
        ...
        self.stop_capture.argtypes = (ctypes.c_int32,)

        self.save_data = self.xenicsdll.XC_SaveData
        self.save_data.restype = ctypes.c_ulong  # ErrCode
        self.save_data.argtypes = (ctypes.c_int32, ctypes.c_char_p, ctypes.c_ulong)
        ...
        self.load_calibration.restype = ctypes.c_ulong  # ErrCode
        # load_calibration.argtypes = (ctypes.c_int32, ctypes.c_char_p, ctypes.c_ulong)

        self.load_settings = self.xenicsdll.XC_LoadSettings
        self.load_settings.restype = ctypes.c_ulong  # ErrCode
        # self.load_settings.argtypes = (ctypes.c_int32, ctypes.c_char_p, ctypes.c_ulong)

        # ColourProfile
        self.load_colour_profile = self.xenicsdll.XC_LoadColourProfile
        self.load_colour_profile.restype = ctypes.c_ulong
        self.load_colour_profile.argtypes = (ctypes.c_char_p,)

        # FileAccessCorrectionFile
        self.set_property_value = self.xenicsdll.XC_SetPropertyValue
        self.set_property_value.restype = ctypes.c_ulong  # ErrCode
    ...
    def get_frame(self, filename):
        ...
        while True:
            ...
            error = self.get_frame_dll(
                self.xenics_handle, frame_t, 1, frame_buffer, size  # Blocking
            )
            print(error)
        ...
        im = np.frombuffer(frame, dtype=pixel_dtype, count=int(size / pixel_size_bytes))
